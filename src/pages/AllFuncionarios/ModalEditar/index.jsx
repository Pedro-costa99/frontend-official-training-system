...
const ModalEditar = ({statusEditFunc, showDois, onValueChange, editFuncionarioDetails, loading, handleCloseDois, funcionario }) => {
    return (
        <Modal show={showDois} onHide={handleCloseDois} className={styles.modalTop} centered>
            ...
            <Modal.Footer className={styles.modalFooter}>
                ...
                <Button onClick={() => editFuncionarioDetails()} variant="primary" className={styles.buttonSalvar}>
                    Salvar alterações
                </Button>
            </Modal.Footer>
            {loading === true ? <div className={styles.msgSucesso}>Alterações salvas com sucesso!</div> : null}
            {statusEditFunc.status === 400 ? <div className={styles.msgErroFunc}>Erro ao alterar os dados</div> : null}
        </Modal>
    );
};
