...
function AllTraining({ itensPerPageTreina, setItensPerPageTreina }) {
    ...
    (async () => {
      await getTreinamentos()
        .then((res) => {
          ...
          console.log(
            "res.data.treinamentos.",
            res.data.treinamentos.reverse(),
          );
  ...
  useEffect(() => {}, [statusEdit, itensPerPageTreina]);
  ...
  const TreinamentoData = useMemo(() => {
    ...
    computedTreinamentos = computedTreinamentos.filter(
      (value) => value.id !== 0,
    );
    ...
    return computedTreinamentos.slice(
      (currentPage - 1) * itensPerPageTreina,
      (currentPage - 1) * itensPerPageTreina + itensPerPageTreina,
    );
  }, [listaTreinamento, currentPage, search, itensPerPageTreina]);
  ...
  const handleLoadMore = () => {
    setItensPerPageTreina(itensPerPageTreina + 2);
  };

  const handleLoadMenos = () => {
    setItensPerPageTreina(2);
  };
  ...
  const renderData = () => (
    <Row className={styles.rowMain}>
      {TreinamentoData.length === 0 ? (
        <span className={styles.spanNaohaTrein}>Não há treinamento</span>
      ) : (
            ...
            >
              <div className={styles.containerIcons}>
                <div className={styles.icons}>
                  <AiTwotoneEdit
                    onClick={() => loadTreinamentoData(value.id)}
                  />
                </div>
                <div className={styles.icons}>
                  <AiFillDelete
                    onClick={() => deleteTreinamentoData(value.id)}
                  />
                </div>
              </div>
              ...
              <Card.Body>
                ...
                <Card.Title className={styles.cardTitle}>
                  Educador (a): {value.educador}
                </Card.Title>
                <Card.Title className={styles.cardTitle}>
                  Carga Horária: {value.ch}
                </Card.Title>
              </Card.Body>
            </Card>
          </Col>
    ...
    <>
      ...
      <main className={styles.main}>
        <div className={styles.mainTop}>
          <div
            className={cx(styles.mb2, "row d-flex justify-content-center")}
            id="mb-2"
          >
            <div
              className={cx(
                styles.totalTreinamentos,
                "col-12 col-sm-6 d-flex justify-content-center",
              )}
            ...
            >
              <Button
                className={styles.buttonCriarTreinamento}
                onClick={handleShow}
                variant="secondary"
                size="lg"
              >
        ...
        />
        <div className={styles.mainBottom}>{renderData()}</div>
        <div className={styles.containerButtonVerMais}>
          {totalItems > 2 &&
          itensPerPageTreina <= TreinamentoData.length &&
          !search ? (
            <button
              type="button"
              onClick={handleLoadMore}
              className={styles.buttonVerMais}
            >
              Ver mais
            </button>
          ) : totalItems > 2 &&
            ...
            <button
              type="button"
              onClick={handleLoadMenos}
              className={styles.buttonSemDados}
            >
              Topo
            </button>
          ) : null}
      ...
      <ModalTreinamento
        listaTreinamento={listaTreinamento}
        show={show}
        setShow={setShow}
        setTreinamento={setTreinamento}
        treinamento={treinamento}
        setListaTreinamento={setListaTreinamento}
      ...
      <ModalEditar
        ...
        editTreinamentoDetails={editTreinamentoDetails}
        loading={loading}
        handleCloseDois={handleCloseDois}
        treinamento={treinamento}
      />
      <ModalDelete
        statusDelTrein={statusDelTrein}
        showTres={showTres}
        handleCloseTres={handleCloseTres}
        loading={loading}
        deleteItemConfirmacao={deleteItemConfirmacao}
      />
    </>
  );
}
